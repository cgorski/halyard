//! What the executor does instead of panicking (README, "Project policy": no panics,
...
fn report(error: &Unspawned) {
    ...
    web_sys::console::error_1(&format!("[halyard] {error} {ONCE}").into());
    #[cfg(all(
        not(feature = "tracing"),
    ...
    ))]
    {
        ...
        // error, it is dropped
        _ = writeln!(std::io::stderr(), "[halyard] {error} {ONCE}");
    }
}
