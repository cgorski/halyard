//! Spawns tasks on one global executor, so that asynchronous code (effects, resources, actions)
//! does not depend on a particular executor.
//!
...
//!
//! Two executors are built in: Tokio on native targets (for the server) and
//! `wasm-bindgen-futures` (for the browser). Any other executor or runtime that supports
//! spawning [`Future`]s can be plugged in with a [`CustomExecutor`].
//!
...
impl Executor {
    ...
    /// dropped without running, and the first such task is logged (see the module's
    /// documentation).
    #[inline(always)]
    #[track_caller]
...
impl Executor {
    ...
    ///
    /// Available on native targets (not on `wasm32`).
    #[cfg(not(target_family = "wasm"))]
