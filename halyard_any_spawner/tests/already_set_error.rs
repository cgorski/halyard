...
fn test_already_set_error() {
    ...
    impl halyard_any_spawner::CustomExecutor for SimpleExecutor {
        fn spawn(&self, _fut: halyard_any_spawner::PinnedFuture<()>) {}
        fn spawn_local(
            &self,
            _fut: halyard_any_spawner::PinnedLocalFuture<()>,
        ) {
        }
        fn poll_local(&self) {}
    }
