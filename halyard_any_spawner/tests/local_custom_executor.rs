...
fn test_local_custom_executor() {
    ...
    impl halyard_any_spawner::CustomExecutor for LocalTestExecutor {
        fn spawn(&self, fut: halyard_any_spawner::PinnedFuture<()>) {
            self.spawn_called.store(true, Ordering::SeqCst);
            futures::executor::block_on(fut);
        }

        fn spawn_local(&self, fut: halyard_any_spawner::PinnedLocalFuture<()>) {
