...
macro_rules! tuples {
    ...
    ($name:ident {
        ...
        #[doc = concat!("Holds a value of one of the types `", stringify!($($ty),+), "`.")]
        #[derive(Debug, PartialEq, Eq, Clone, Copy, Hash)]
        pub enum $name<$($ty),+> {
            $(
                #[doc = concat!("Holds a `", stringify!($ty), "`.")]
                $variant ($ty),
            )+
        }
        ...
        impl<$($ty),+> $name<$($ty),+> {
            paste! {
                /// Maps the value with the function given for its variant.
                ...
                $(
                    #[doc = concat!("Maps a `", stringify!($variant), "` value with `f`; other variants stay as they are.")]
                    ...
                    #[doc = concat!("Calls `f` with a `", stringify!($variant), "` value, and returns `self`.")]
                    ...
                    #[doc = concat!("Whether this is a `", stringify!($variant), "`.")]
                    pub fn [<is_ $variant:lower>](&self) -> bool {
                        matches!(self, $name::$variant(_))
                    }

                    #[doc = concat!("The `", stringify!($variant), "` value, if this is one.")]
                    ...
                    #[doc = concat!("The `", stringify!($variant), "` value, mutably, if this is one.")]
                    ...
                    #[doc = concat!("The `", stringify!($variant), "` value.")]
                    ///
                    /// # Panics
                    /// Panics if this is another variant.
                    ...
                    #[doc = concat!("The `", stringify!($variant), "` value, or `self` back if this is another variant.")]
...
impl<A, B> Either<A, B> {
    /// Swaps the variants: `Left(a)` becomes `Right(a)`, and `Right(b)` becomes `Left(b)`.
...
/// Turns a value with two cases (such as a `bool`, an `Option` or a `Result`) into an
/// [`Either`], mapping each case with its own function.
pub trait EitherOr {
    /// What the first case holds.
    type Left;
    /// What the second case holds.
    type Right;
    /// Maps the first case with `a` into `Left`, and the second with `b` into `Right`.
...
macro_rules! either {
    ($match:expr, $left_pattern:pat => $left_expression:expr, $right_pattern:pat => $right_expression:expr$(,)?) => {
        match $match {
            $left_pattern => $crate::either::Either::Left($left_expression),
            $right_pattern => $crate::either::Either::Right($right_expression),
        }
    };
    ($match:expr, $a_pattern:pat => $a_expression:expr, $b_pattern:pat => $b_expression:expr, $c_pattern:pat => $c_expression:expr$(,)?) => {
        match $match {
            $a_pattern => $crate::either::EitherOf3::A($a_expression),
            $b_pattern => $crate::either::EitherOf3::B($b_expression),
            $c_pattern => $crate::either::EitherOf3::C($c_expression),
        }
    };
    ($match:expr, $a_pattern:pat => $a_expression:expr, $b_pattern:pat => $b_expression:expr, $c_pattern:pat => $c_expression:expr, $d_pattern:pat => $d_expression:expr$(,)?) => {
        match $match {
            $a_pattern => $crate::either::EitherOf4::A($a_expression),
            $b_pattern => $crate::either::EitherOf4::B($b_expression),
            $c_pattern => $crate::either::EitherOf4::C($c_expression),
            $d_pattern => $crate::either::EitherOf4::D($d_expression),
        }
    };
    ($match:expr, $a_pattern:pat => $a_expression:expr, $b_pattern:pat => $b_expression:expr, $c_pattern:pat => $c_expression:expr, $d_pattern:pat => $d_expression:expr, $e_pattern:pat => $e_expression:expr$(,)?) => {
        match $match {
            $a_pattern => $crate::either::EitherOf5::A($a_expression),
            $b_pattern => $crate::either::EitherOf5::B($b_expression),
            $c_pattern => $crate::either::EitherOf5::C($c_expression),
            $d_pattern => $crate::either::EitherOf5::D($d_expression),
            $e_pattern => $crate::either::EitherOf5::E($e_expression),
        }
    };
    ($match:expr, $a_pattern:pat => $a_expression:expr, $b_pattern:pat => $b_expression:expr, $c_pattern:pat => $c_expression:expr, $d_pattern:pat => $d_expression:expr, $e_pattern:pat => $e_expression:expr, $f_pattern:pat => $f_expression:expr$(,)?) => {
        match $match {
            $a_pattern => $crate::either::EitherOf6::A($a_expression),
            $b_pattern => $crate::either::EitherOf6::B($b_expression),
            $c_pattern => $crate::either::EitherOf6::C($c_expression),
            $d_pattern => $crate::either::EitherOf6::D($d_expression),
            $e_pattern => $crate::either::EitherOf6::E($e_expression),
            $f_pattern => $crate::either::EitherOf6::F($f_expression),
        }
    };
    ($match:expr, $a_pattern:pat => $a_expression:expr, $b_pattern:pat => $b_expression:expr, $c_pattern:pat => $c_expression:expr, $d_pattern:pat => $d_expression:expr, $e_pattern:pat => $e_expression:expr, $f_pattern:pat => $f_expression:expr, $g_pattern:pat => $g_expression:expr$(,)?) => {
        match $match {
            $a_pattern => $crate::either::EitherOf7::A($a_expression),
            $b_pattern => $crate::either::EitherOf7::B($b_expression),
            $c_pattern => $crate::either::EitherOf7::C($c_expression),
            $d_pattern => $crate::either::EitherOf7::D($d_expression),
            $e_pattern => $crate::either::EitherOf7::E($e_expression),
            $f_pattern => $crate::either::EitherOf7::F($f_expression),
            $g_pattern => $crate::either::EitherOf7::G($g_expression),
        }
    };
    ($match:expr, $a_pattern:pat => $a_expression:expr, $b_pattern:pat => $b_expression:expr, $c_pattern:pat => $c_expression:expr, $d_pattern:pat => $d_expression:expr, $e_pattern:pat => $e_expression:expr, $f_pattern:pat => $f_expression:expr, $g_pattern:pat => $g_expression:expr, $h_pattern:pat => $h_expression:expr$(,)?) => {
        match $match {
            $a_pattern => $crate::either::EitherOf8::A($a_expression),
            $b_pattern => $crate::either::EitherOf8::B($b_expression),
            $c_pattern => $crate::either::EitherOf8::C($c_expression),
            $d_pattern => $crate::either::EitherOf8::D($d_expression),
            $e_pattern => $crate::either::EitherOf8::E($e_expression),
            $f_pattern => $crate::either::EitherOf8::F($f_expression),
            $g_pattern => $crate::either::EitherOf8::G($g_expression),
            $h_pattern => $crate::either::EitherOf8::H($h_expression),
        }
    };
    ($match:expr, $a_pattern:pat => $a_expression:expr, $b_pattern:pat => $b_expression:expr, $c_pattern:pat => $c_expression:expr, $d_pattern:pat => $d_expression:expr, $e_pattern:pat => $e_expression:expr, $f_pattern:pat => $f_expression:expr, $g_pattern:pat => $g_expression:expr, $h_pattern:pat => $h_expression:expr, $i_pattern:pat => $i_expression:expr$(,)?) => {
        match $match {
            $a_pattern => $crate::either::EitherOf9::A($a_expression),
            $b_pattern => $crate::either::EitherOf9::B($b_expression),
            $c_pattern => $crate::either::EitherOf9::C($c_expression),
            $d_pattern => $crate::either::EitherOf9::D($d_expression),
            $e_pattern => $crate::either::EitherOf9::E($e_expression),
            $f_pattern => $crate::either::EitherOf9::F($f_expression),
            $g_pattern => $crate::either::EitherOf9::G($g_expression),
            $h_pattern => $crate::either::EitherOf9::H($h_expression),
            $i_pattern => $crate::either::EitherOf9::I($i_expression),
        }
    };
    ($match:expr, $a_pattern:pat => $a_expression:expr, $b_pattern:pat => $b_expression:expr, $c_pattern:pat => $c_expression:expr, $d_pattern:pat => $d_expression:expr, $e_pattern:pat => $e_expression:expr, $f_pattern:pat => $f_expression:expr, $g_pattern:pat => $g_expression:expr, $h_pattern:pat => $h_expression:expr, $i_pattern:pat => $i_expression:expr, $j_pattern:pat => $j_expression:expr$(,)?) => {
        match $match {
            $a_pattern => $crate::either::EitherOf10::A($a_expression),
            $b_pattern => $crate::either::EitherOf10::B($b_expression),
            $c_pattern => $crate::either::EitherOf10::C($c_expression),
            $d_pattern => $crate::either::EitherOf10::D($d_expression),
            $e_pattern => $crate::either::EitherOf10::E($e_expression),
            $f_pattern => $crate::either::EitherOf10::F($f_expression),
            $g_pattern => $crate::either::EitherOf10::G($g_expression),
            $h_pattern => $crate::either::EitherOf10::H($h_expression),
            $i_pattern => $crate::either::EitherOf10::I($i_expression),
            $j_pattern => $crate::either::EitherOf10::J($j_expression),
        }
    };
    ($match:expr, $a_pattern:pat => $a_expression:expr, $b_pattern:pat => $b_expression:expr, $c_pattern:pat => $c_expression:expr, $d_pattern:pat => $d_expression:expr, $e_pattern:pat => $e_expression:expr, $f_pattern:pat => $f_expression:expr, $g_pattern:pat => $g_expression:expr, $h_pattern:pat => $h_expression:expr, $i_pattern:pat => $i_expression:expr, $j_pattern:pat => $j_expression:expr, $k_pattern:pat => $k_expression:expr$(,)?) => {
        match $match {
            $a_pattern => $crate::either::EitherOf11::A($a_expression),
            $b_pattern => $crate::either::EitherOf11::B($b_expression),
            $c_pattern => $crate::either::EitherOf11::C($c_expression),
            $d_pattern => $crate::either::EitherOf11::D($d_expression),
            $e_pattern => $crate::either::EitherOf11::E($e_expression),
            $f_pattern => $crate::either::EitherOf11::F($f_expression),
            $g_pattern => $crate::either::EitherOf11::G($g_expression),
            $h_pattern => $crate::either::EitherOf11::H($h_expression),
            $i_pattern => $crate::either::EitherOf11::I($i_expression),
            $j_pattern => $crate::either::EitherOf11::J($j_expression),
            $k_pattern => $crate::either::EitherOf11::K($k_expression),
        }
    };
    ($match:expr, $a_pattern:pat => $a_expression:expr, $b_pattern:pat => $b_expression:expr, $c_pattern:pat => $c_expression:expr, $d_pattern:pat => $d_expression:expr, $e_pattern:pat => $e_expression:expr, $f_pattern:pat => $f_expression:expr, $g_pattern:pat => $g_expression:expr, $h_pattern:pat => $h_expression:expr, $i_pattern:pat => $i_expression:expr, $j_pattern:pat => $j_expression:expr, $k_pattern:pat => $k_expression:expr, $l_pattern:pat => $l_expression:expr$(,)?) => {
        match $match {
            $a_pattern => $crate::either::EitherOf12::A($a_expression),
            $b_pattern => $crate::either::EitherOf12::B($b_expression),
            $c_pattern => $crate::either::EitherOf12::C($c_expression),
            $d_pattern => $crate::either::EitherOf12::D($d_expression),
            $e_pattern => $crate::either::EitherOf12::E($e_expression),
            $f_pattern => $crate::either::EitherOf12::F($f_expression),
            $g_pattern => $crate::either::EitherOf12::G($g_expression),
            $h_pattern => $crate::either::EitherOf12::H($h_expression),
            $i_pattern => $crate::either::EitherOf12::I($i_expression),
            $j_pattern => $crate::either::EitherOf12::J($j_expression),
            $k_pattern => $crate::either::EitherOf12::K($k_expression),
            $l_pattern => $crate::either::EitherOf12::L($l_expression),
        }
    };
    ($match:expr, $a_pattern:pat => $a_expression:expr, $b_pattern:pat => $b_expression:expr, $c_pattern:pat => $c_expression:expr, $d_pattern:pat => $d_expression:expr, $e_pattern:pat => $e_expression:expr, $f_pattern:pat => $f_expression:expr, $g_pattern:pat => $g_expression:expr, $h_pattern:pat => $h_expression:expr, $i_pattern:pat => $i_expression:expr, $j_pattern:pat => $j_expression:expr, $k_pattern:pat => $k_expression:expr, $l_pattern:pat => $l_expression:expr, $m_pattern:pat => $m_expression:expr$(,)?) => {
        match $match {
            $a_pattern => $crate::either::EitherOf13::A($a_expression),
            $b_pattern => $crate::either::EitherOf13::B($b_expression),
            $c_pattern => $crate::either::EitherOf13::C($c_expression),
            $d_pattern => $crate::either::EitherOf13::D($d_expression),
            $e_pattern => $crate::either::EitherOf13::E($e_expression),
            $f_pattern => $crate::either::EitherOf13::F($f_expression),
            $g_pattern => $crate::either::EitherOf13::G($g_expression),
            $h_pattern => $crate::either::EitherOf13::H($h_expression),
            $i_pattern => $crate::either::EitherOf13::I($i_expression),
            $j_pattern => $crate::either::EitherOf13::J($j_expression),
            $k_pattern => $crate::either::EitherOf13::K($k_expression),
            $l_pattern => $crate::either::EitherOf13::L($l_expression),
            $m_pattern => $crate::either::EitherOf13::M($m_expression),
        }
    };
    ($match:expr, $a_pattern:pat => $a_expression:expr, $b_pattern:pat => $b_expression:expr, $c_pattern:pat => $c_expression:expr, $d_pattern:pat => $d_expression:expr, $e_pattern:pat => $e_expression:expr, $f_pattern:pat => $f_expression:expr, $g_pattern:pat => $g_expression:expr, $h_pattern:pat => $h_expression:expr, $i_pattern:pat => $i_expression:expr, $j_pattern:pat => $j_expression:expr, $k_pattern:pat => $k_expression:expr, $l_pattern:pat => $l_expression:expr, $m_pattern:pat => $m_expression:expr, $n_pattern:pat => $n_expression:expr$(,)?) => {
        match $match {
            $a_pattern => $crate::either::EitherOf14::A($a_expression),
            $b_pattern => $crate::either::EitherOf14::B($b_expression),
            $c_pattern => $crate::either::EitherOf14::C($c_expression),
            $d_pattern => $crate::either::EitherOf14::D($d_expression),
            $e_pattern => $crate::either::EitherOf14::E($e_expression),
            $f_pattern => $crate::either::EitherOf14::F($f_expression),
            $g_pattern => $crate::either::EitherOf14::G($g_expression),
            $h_pattern => $crate::either::EitherOf14::H($h_expression),
            $i_pattern => $crate::either::EitherOf14::I($i_expression),
            $j_pattern => $crate::either::EitherOf14::J($j_expression),
            $k_pattern => $crate::either::EitherOf14::K($k_expression),
            $l_pattern => $crate::either::EitherOf14::L($l_expression),
            $m_pattern => $crate::either::EitherOf14::M($m_expression),
            $n_pattern => $crate::either::EitherOf14::N($n_expression),
        }
    };
    ($match:expr, $a_pattern:pat => $a_expression:expr, $b_pattern:pat => $b_expression:expr, $c_pattern:pat => $c_expression:expr, $d_pattern:pat => $d_expression:expr, $e_pattern:pat => $e_expression:expr, $f_pattern:pat => $f_expression:expr, $g_pattern:pat => $g_expression:expr, $h_pattern:pat => $h_expression:expr, $i_pattern:pat => $i_expression:expr, $j_pattern:pat => $j_expression:expr, $k_pattern:pat => $k_expression:expr, $l_pattern:pat => $l_expression:expr, $m_pattern:pat => $m_expression:expr, $n_pattern:pat => $n_expression:expr, $o_pattern:pat => $o_expression:expr$(,)?) => {
        match $match {
            $a_pattern => $crate::either::EitherOf15::A($a_expression),
            $b_pattern => $crate::either::EitherOf15::B($b_expression),
            $c_pattern => $crate::either::EitherOf15::C($c_expression),
            $d_pattern => $crate::either::EitherOf15::D($d_expression),
            $e_pattern => $crate::either::EitherOf15::E($e_expression),
            $f_pattern => $crate::either::EitherOf15::F($f_expression),
            $g_pattern => $crate::either::EitherOf15::G($g_expression),
            $h_pattern => $crate::either::EitherOf15::H($h_expression),
            $i_pattern => $crate::either::EitherOf15::I($i_expression),
            $j_pattern => $crate::either::EitherOf15::J($j_expression),
            $k_pattern => $crate::either::EitherOf15::K($k_expression),
            $l_pattern => $crate::either::EitherOf15::L($l_expression),
            $m_pattern => $crate::either::EitherOf15::M($m_expression),
            $n_pattern => $crate::either::EitherOf15::N($n_expression),
            $o_pattern => $crate::either::EitherOf15::O($o_expression),
        }
    };
    ($match:expr, $a_pattern:pat => $a_expression:expr, $b_pattern:pat => $b_expression:expr, $c_pattern:pat => $c_expression:expr, $d_pattern:pat => $d_expression:expr, $e_pattern:pat => $e_expression:expr, $f_pattern:pat => $f_expression:expr, $g_pattern:pat => $g_expression:expr, $h_pattern:pat => $h_expression:expr, $i_pattern:pat => $i_expression:expr, $j_pattern:pat => $j_expression:expr, $k_pattern:pat => $k_expression:expr, $l_pattern:pat => $l_expression:expr, $m_pattern:pat => $m_expression:expr, $n_pattern:pat => $n_expression:expr, $o_pattern:pat => $o_expression:expr, $p_pattern:pat => $p_expression:expr$(,)?) => {
        match $match {
            $a_pattern => $crate::either::EitherOf16::A($a_expression),
            $b_pattern => $crate::either::EitherOf16::B($b_expression),
            $c_pattern => $crate::either::EitherOf16::C($c_expression),
            $d_pattern => $crate::either::EitherOf16::D($d_expression),
            $e_pattern => $crate::either::EitherOf16::E($e_expression),
            $f_pattern => $crate::either::EitherOf16::F($f_expression),
            $g_pattern => $crate::either::EitherOf16::G($g_expression),
            $h_pattern => $crate::either::EitherOf16::H($h_expression),
            $i_pattern => $crate::either::EitherOf16::I($i_expression),
            $j_pattern => $crate::either::EitherOf16::J($j_expression),
            $k_pattern => $crate::either::EitherOf16::K($k_expression),
            $l_pattern => $crate::either::EitherOf16::L($l_expression),
            $m_pattern => $crate::either::EitherOf16::M($m_expression),
            $n_pattern => $crate::either::EitherOf16::N($n_expression),
            $o_pattern => $crate::either::EitherOf16::O($o_expression),
            $p_pattern => $crate::either::EitherOf16::P($p_expression),
        }
    };
}
...
///
/// let _: halyard_tachys::either::EitherOf3<u8, i8, u32> = <Either!(u8, i8, u32)>::A(0);
/// ```
