...
impl ServerFnCall {
    ...
    ///
    /// This runs when tokens are emitted (not in `parse`), so that it uses
    /// the `server_fn_path` configured by the calling macro rather than the
    /// bare `server_fn` default.
    fn actix_send_wrapped_body(&self) -> ServerFnBody {
    ...
    pub fn server_fn_path(&self) -> Path {
        self.server_fn_path
            .clone()
            .unwrap_or_else(|| parse_quote! { ::halyard_server_fn })
    }
...
impl ToTokens for ServerFnCall {
    fn to_tokens(&self, tokens: &mut TokenStream2) {
        let body = self.actix_send_wrapped_body();
