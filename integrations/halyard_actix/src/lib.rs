...
//! Provides functions to easily integrate Halyard with Actix.
//!
...
/// `Send`/`Sync` setting like Halyard's Context API.
#[derive(Debug, Clone)]
...
///    to using [`ActionForm`](halyard::form::ActionForm) without JS/WASM present.)
///
/// Using it with a non-blocking [`Resource`](halyard::server::Resource) will not work if you are using streaming rendering,
...
/// Halyard server function arguments in the body, runs the server function if found,
...
///             .route("/api/{tail:.*}", halyard_actix::handle_server_fns())
///     })
///     .bind(("127.0.0.1", 8080))?
///     .run()
///     .await
...
/// Halyard server function arguments in the URL (`GET`) or body (`POST`),
...
/// [`handle_server_fns_with_context`] **and** in [`HalyardRoutes::halyard_routes_with_context`] (or whatever
    ...
    web::to(move |req: HttpRequest, payload: Payload| {
        ...
        async move {
            ...
            if let Some(mut service) =
                halyard_server_fn::actix::get_server_fn_service(path, method)
            {
                let owner = Owner::new();
                owner
                    .with(|| {
...
/// to route it using [halyard_router], serving an HTML stream of your application. The stream
...
///     let addr = conf.halyard_options.site_addr.clone();
///     HttpServer::new(move || {
///         let halyard_options = &conf.halyard_options;
///
///         App::new()
///             // {tail:.*} passes the remainder of the URL as the route
///             // the actual routing will be handled by `halyard_router`
///             .route(
///                 "/{tail:.*}",
///                 halyard_actix::render_app_to_stream(MyApp, Method::Get),
///             )
///     })
///     .bind(&addr)?
///     .run()
...
/// - [ResponseOptions]
/// - [Request]
/// - [MetaContext](halyard_meta::MetaContext)
#[cfg_attr(
    feature = "tracing",
    tracing::instrument(level = "trace", fields(error), skip_all)
)]
...
///             .route(
///                 "/{tail:.*}",
///                 halyard_actix::render_app_to_stream_in_order(
///                     MyApp,
///                     Method::Get,
///                 ),
///             )
...
///             .route(
///                 "/{tail:.*}",
///                 halyard_actix::render_app_async(MyApp, Method::Get),
///             )
///     })
///     .bind(&addr)?
///     .run()
...
///
/// This function allows you to provide additional information to Halyard for your route.
...
/// Returns an Actix [struct@Route](actix_web::Route) that listens for a `GET` request and tries
/// to route it using [halyard_router], asynchronously serving the page once all `async`
...
fn provide_contexts(
    ...
    let path = halyard_corrected_path(&req);
...
fn halyard_corrected_path(req: &HttpRequest) -> String {
    let path = req.path();
    let query = req.query_string();
    if query.is_empty() {
        "http://halyard".to_string() + path
    } else {
        "http://halyard".to_string() + path + "?" + query
    }
}
...
/// to this function will stop `.halyard_routes()` from generating a route for it, allowing a custom handler. These need to be in Actix path format
...
where
    IV: IntoView + 'static,
{
    let handler = move |req: HttpRequest, data: Data<HalyardOptions>| {
        Box::pin({
            let app_fn = app_fn.clone();
            let additional_context = additional_context.clone();
...
impl<T> HalyardRoutes for actix_web::App<T>
where
    T: ServiceFactory<
        ServiceRequest,
        Config = (),
    ...
    >,
{
    ...
    where
        IV: IntoView + 'static,
    {
        self.halyard_routes_with_context(paths, || {}, app_fn)
    }
        ...
        IV: IntoView + 'static,
    {
        ...
        // register server functions first to allow for wildcard route in Halyard's Router
        for (path, _) in halyard_server_fn::actix::server_fn_paths() {
        ...
        // register routes defined in Halyard's Router
...
/// The default implementation of `HalyardRoutes` which takes in a list of paths, and dispatches GET requests
/// to those paths to Halyard's renderer.
impl HalyardRoutes for &mut ServiceConfig {
    #[cfg_attr(
        feature = "tracing",
        tracing::instrument(level = "trace", fields(error), skip_all)
    )]
