...
use crate::helper_error::HelperError;
use halyard_or_poisoned::OrPoisoned;
#[cfg(debug_assertions)]
use halyard_reactive_graph::diagnostics::SpecialNonReactiveZone;
use halyard_reactive_graph::owner::Owner;
use halyard_tachys::html::event::EventDescriptor;
...
///
/// An event without a target (created, but never dispatched) gives an empty string, and a
/// warning in the console.
pub fn event_target_value<T>(event: &T) -> String
where
    T: JsCast,
{
    match event.unchecked_ref::<web_sys::Event>().target() {
        Some(target) => {
            target.unchecked_into::<web_sys::HtmlInputElement>().value()
        }
        None => {
            HelperError::NoEventTarget
                .warn("`event_target_value` returns an empty string.");
            String::new()
        }
    }
}
...
///
/// An event without a target (created, but never dispatched) gives `false`, and a warning
/// in the console.
pub fn event_target_checked(ev: &web_sys::Event) -> bool {
    match ev.target() {
        ...
        None => {
            HelperError::NoEventTarget
                .warn("`event_target_checked` returns `false`.");
            false
        }
    }
}
...
/// ### Note about Context
///
/// The callback is called outside of the reactive ownership tree. This means that it does not have access to context via [`use_context`](halyard_reactive_graph::owner::use_context). If you want to use context inside the callback, you should either call `use_context` in the body of the component, and move the value into the callback, or access the current owner inside the component body using [`Owner::current`](halyard_reactive_graph::owner::Owner::current) and reestablish it in the callback with [`Owner::with`](halyard_reactive_graph::owner::Owner::with).
...
pub fn queue_microtask(task: impl FnOnce() + 'static) {
    halyard_tachys::renderer::dom::queue_microtask(task);
}
...
pub fn set_timeout(cb: impl FnOnce() + 'static, duration: Duration) {
    if let Err(thrown) = set_timeout_with_handle(cb, duration) {
        HelperError::thrown("setTimeout", &thrown)
            .warn("The callback is not called.");
    }
}
...
/// [`setTimeout()`](https://developer.mozilla.org/en-US/docs/Web/API/setTimeout).
///
/// Returns an error if the browser refuses, or if `duration` is longer than the browser's
/// timers can wait (`i32::MAX` milliseconds, about 24.8 days).
///
/// ### Note about Context
///
/// The callback is called outside of the reactive ownership tree. This means that it does not have access to context via [`use_context`](halyard_reactive_graph::owner::use_context). If you want to use context inside the callback, you should either call `use_context` in the body of the component, and move the value into the callback, or access the current owner inside the component body using [`Owner::current`](halyard_reactive_graph::owner::Owner::current) and reestablish it in the callback with [`Owner::with`](halyard_reactive_graph::owner::Owner::with).
...
///
/// ```
/// use halyard::{halyard_dom::helpers::debounce, logging::log, prelude::*, *};
///
/// #[component]
...
pub fn debounce<T: 'static>(
    ...
    // `None` while the callback runs: it is taken out, so that no lock is held while user
    // code runs (docs/no-panics.md, change 2)
    let cb = Arc::new(RwLock::new(Some(cb)));
        ...
        let handle = set_timeout_with_handle(
            {
                let cb = Arc::clone(&cb);
                move || {
                    let taken = cb.write().or_poisoned().take();
                    let Some(mut run) = taken else {
                        HelperError::DebounceReentered
                            .warn("This call is skipped.");
                        return;
                    };
                    run(arg);
                    *cb.write().or_poisoned() = Some(run);
                }
            },
            delay,
        );
        match handle {
            Ok(handle) => *timer.write().or_poisoned() = Some(handle),
            Err(thrown) => HelperError::thrown("setTimeout", &thrown)
                .warn("The debounced callback is not called."),
        }
    }
}
...
pub fn set_interval(cb: impl Fn() + 'static, duration: Duration) {
    if let Err(thrown) = set_interval_with_handle(cb, duration) {
        HelperError::thrown("setInterval", &thrown)
            .warn("The callback is not called.");
    }
}
...
pub fn window_event_listener_untyped(
    ...
    if !is_server() {
        ...
        fn wel(
            ...
            let cb = Closure::wrap(cb).into_js_value();
            if let Err(thrown) = window().add_event_listener_with_callback(
                event_name,
                cb.unchecked_ref(),
            ) {
                HelperError::thrown("window.addEventListener", &thrown).warn(
                    &format!("There is no window `{event_name}` listener."),
                );
            }
...
/// ```
/// use halyard::{
///     ev, halyard_dom::helpers::window_event_listener, logging::log,
///     prelude::*,
/// };
///
/// #[component]
...
/// `duration` as the whole milliseconds that `setTimeout` and `setInterval` take.
fn timer_delay(duration: Duration) -> Result<i32, HelperError> {
    let millis = duration.as_millis();
    i32::try_from(millis).map_err(|_| HelperError::DelayTooLong { millis })
}
...
mod tests {
    ...
    /// A delay longer than the browser's timers can count (about 24.8 days) was
    /// `.try_into().unwrap_throw()` in `set_timeout` and `set_interval`: a thrown
    /// exception through the app. It is now an error that the timer helpers return (or
    /// log, for `set_timeout` and `set_interval`, which return nothing).
    #[test]
    fn a_delay_the_browser_cannot_wait_is_an_error() {
        let longest = u64::from(i32::MAX.unsigned_abs());

        assert!(matches!(
            timer_delay(Duration::from_millis(longest + 1)),
            Err(HelperError::DelayTooLong {
                millis: 2_147_483_648
            })
        ));
        assert!(matches!(
            timer_delay(Duration::MAX),
            Err(HelperError::DelayTooLong { .. })
        ));
    }

    /// Delays the browser can wait are passed as whole milliseconds, as before.
    #[test]
    fn a_delay_the_browser_can_wait_is_in_whole_milliseconds() {
        let longest = u64::from(i32::MAX.unsigned_abs());

        assert_eq!(timer_delay(Duration::ZERO).ok(), Some(0));
        assert_eq!(timer_delay(Duration::from_micros(1_999)).ok(), Some(1));
        assert_eq!(timer_delay(Duration::from_millis(250)).ok(), Some(250));
        assert_eq!(
            timer_delay(Duration::from_millis(longest)).ok(),
            Some(i32::MAX)
        );
    }
}
