...
/// Minimal stand-in for `proc_macro_error2::Diagnostic`: `.emit()` records
/// the error and lets the derive continue.
...
trait SpanError: Spanned {
    fn error(&self, text: &str) -> Diagnostic {
        Diagnostic(syn::Error::new(self.span(), text))
    }
}
...
fn derive_tokens_inner(input: DeriveInput) -> TokenStream {
    let body = match &input.data {
        Data::Struct(data) => {
            let (pat, body) = derive_tokens_fields(
                pq! {_=> Self },
                input.span(),
                &data.fields,
            );
        ...
        for attr in &field.attrs {
            if attr.path().is_ident("parse") {
                if fa.delimiter.is_some() {
                    fa.delimiter
                        .span()
                        .error("cannot do custom parsing on delimiters")
                        .emit();
                }
...
fn derive_tokens_fields_inner(
    iter: &mut std::iter::Peekable<
        impl Iterator<Item = (syn::Member, FieldAttr, syn::Field)>,
    >,
    stream: Option<Ident>,
    pat: &mut TokenStream,
    ...
    for attr in &field.attrs {
        if attr.path().is_ident("syn") {
            attr.parse_nested_meta(|meta| {
                ...
                if meta.path.is_ident("parenthesized")
                    || meta.path.is_ident("braced")
                    || meta.path.is_ident("bracketed")
                {
...
fn named(
    ...
    fields.iter().cloned().enumerate().map(|(i, f)| {
        (
