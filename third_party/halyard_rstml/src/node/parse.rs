...
impl<C: CustomNode> ParseRecoverable for NodeFragment<C> {
    fn parse_recoverable(
        ...
        let (children, tag_close) = if is_raw("") {
            let (child, closed_tag) = parser.parse_with_ending(
                input,
                |_, t| RawText::from(t),
                FragmentClose::parse,
            );
            ...
            let (child, close_tag_start) = parser
                .parse_tokens_until_call::<Node<C>, _, _>(
                    input,
                    CloseTagStart::parse,
                );
            (
                child,
                FragmentClose::parse_with_start_tag(
                    parser,
                    input,
                    close_tag_start,
                ),
        ...
        let close_tag_start =
            tag_close.as_ref().map(|v| v.start_tag.token_lt.span());
...
impl ParseRecoverable for NodeDoctype {
    fn parse_recoverable(
        ...
        let (value, token_end) = parser.parse_with_ending(
            input,
            |_, t| RawText::from(t),
            <Token![>]>::parse,
        );
...
impl<C: CustomNode> ParseRecoverable for NodeElement<C> {
    fn parse_recoverable(
        ...
        let (children, close_tag) = Self::parse_children(
            parser,
            input,
            is_raw(tag_name_str),
            &open_tag,
...
pub(super) fn parse_array_of2_tokens<T: Parse>(
    input: ParseStream,
) -> syn::Result<[T; 2]> {
        ...
        let parser = move |block_content: ParseStream| {
            ...
            match transform_fn(&forked_block_content) {
                Ok(transformed_tokens) => match transformed_tokens {
                    ...
                    None => {
                        block_expr_with_extern_span(block_content, block_span)
                    }
                },
                Err(error) => Err(error),
            }
...
fn block_expr_with_extern_span(
    input: ParseStream,
    span: DelimSpan,
) -> syn::Result<Block> {
    Ok(Block {
