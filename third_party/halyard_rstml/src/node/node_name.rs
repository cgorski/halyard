...
impl NodeName {
    ...
    pub(crate) fn node_name_punctuated_ident<
        T: Parse,
        F: Peek,
        X: From<Ident>,
    >(
    ...
    >(
        ...
        while !fork.is_empty() {
            ...
            if fork.peek(punct)
                || fork.peek(alternate_punct)
                || fork.peek(alternate_punct2)
            {
                segments.push_punct(fork.parse()?);
            } else {
                break;
...
impl TryFrom<&NodeName> for Block {
    ...
    fn try_from(node: &NodeName) -> Result<Self, Self::Error> {
        match node {
            ...
            _ => Err(Error::TryFrom(
                "NodeName does not match NodeName::Block(Expr::Block(_))"
                    .into(),
            )),
        }
    }
    ...
    fn eq(&self, other: &NodeName) -> bool {
        match self {
            ...
            Self::Punctuated(this) => match other {
                Self::Punctuated(other) => this.pairs().zip(other.pairs()).all(
                    |(this, other)| match (this, other) {
                        (
...
impl Parse for NodeName {
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        if input.peek(LitInt) {
            ...
            NodeName::node_name_punctuated_ident::<
                PathSep,
                fn(_) -> PathSep,
                PathSegment,
            >(input, PathSep)
