...
impl<C> Clone for ParserConfig<C> {
    fn clone(&self) -> Self {
        Self {
            ...
            always_self_closed_elements: self
...
impl<C> ParserConfig<C> {
    ...
    /// Examples:
    /// <br> <link> <img>
    pub fn always_self_closed_elements(
        mut self,
    ...
    pub fn element_close_use_default_wildcard_ident(
        self,
        open_tag_should_be_block: bool,
    ) -> Self {
        self.element_close_wildcard(move |open_tag, close_tag| {
            close_tag.name.is_wildcard()
                && (!open_tag_should_be_block || open_tag.name.is_block())
        })
    }
    ...
    pub fn macro_call_pattern(mut self, pattern: TokenStream) -> Self {
        self.macro_pattern = MacroPattern::from_token_stream(pattern)
            .expect("No %% token found in pattern.");
        self
    }
