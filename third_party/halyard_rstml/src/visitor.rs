...
pub trait Visitor<Custom> {
    ...
    fn visit_raw_node<AnyC: CustomNode>(
        &mut self,
        _node: &mut RawText<AnyC>,
    ) -> bool {
        true
    }
    ...
    fn visit_keyed_attribute(
        &mut self,
        _attribute: &mut KeyedAttribute,
    ) -> bool {
        true
    }
    ...
    CW: CustomNodeWalker<Custom = C>,
{
    ...
    fn visit_raw_node<OtherC: CustomNode>(
        &mut self,
    ...
    fn visit_keyed_attribute(
        &mut self,
        attribute: &mut KeyedAttribute,
    ) -> bool {
        visit_inner!(self.visitor.visit_keyed_attribute(attribute));

        match &mut attribute.possible_value {
            KeyedAttributeValue::None => {
                self.visit_attribute_flag(&mut attribute.key)
            }
            KeyedAttributeValue::Binding(b) => {
                self.visit_attribute_binding(&mut attribute.key, b)
            }
            KeyedAttributeValue::Value(v) => {
                self.visit_attribute_value(&mut attribute.key, v)
            }
        }
    }
    ...
    fn visit_attribute_value(
        ...
        match &mut value.value {
            ...
            KVAttributeValue::InvalidBraced(braced) => {
                self.visit_invalid_block(braced)
            }
        }
    }
...
mod tests {
    ...
    fn collect_node_names() {
        ...
        assert_eq!(
            node_names,
            vec![
                "div", "span", "span", "span", "span", "div", "foo", "key",
                "foo"
            ]
        );
    }
