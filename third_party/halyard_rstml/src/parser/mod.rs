...
impl<C: CustomNode + std::fmt::Debug> Parser<C> {
    ...
    pub fn parse_syn_stream(
        ...
        while !input.is_empty() {
            let Some(parsed_node) = Node::parse_recoverable(&mut parser, input)
            else {
                parser.push_diagnostic(
                    input.error("Node parse failed".to_string()),
                );
                break;
            };

            if let Some(type_of_top_level_nodes) =
                &self.config.type_of_top_level_nodes
            {
        ...
        if let Some(number_of_top_level_nodes) =
            &self.config.number_of_top_level_nodes
        {
