...
pub fn replace_node<C: CustomNode + std::fmt::Debug>(
    source: &mut Node<C>,
    hacked: &Node<C>,
) {
    match (source, hacked) {
        ...
        for tt in stream {
            match tt {
                TokenTree::Group(group) => {
                    if let Some(post_res) =
                        Self::from_token_stream_inner(group.stream())
                    {
                ...
                TokenTree::Punct(p) => {
                    match (p.as_char(), last_token_percent) {
...
mod test {
    ...
    fn macro_content_matcher() {
        let pattern =
            MacroPattern::from_token_stream(quote! {html!{ctx, other_arg, %%}})
                .unwrap();
        let content = pattern
