...
macro_rules! tracing_props {
    () => {
        ::halyard::tracing::span!(
            ::halyard::tracing::Level::TRACE,
            "halyard::tracing_props",
            props = String::from("[]")
        );
    };
    ($($prop:tt),+ $(,)?) => {
        {
            // a glob: each expansion uses one of the two traits, and a glob counts as used
            use ::halyard::dom::macro_helpers::tracing_property::*;
