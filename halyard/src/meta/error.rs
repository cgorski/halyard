//! What `halyard::meta` could not do, logged in place of the panics these replace.

use std::{io, path::PathBuf};
use wasm_bindgen::{JsCast, JsValue};

/// Something `halyard::meta` could not do.
///
...
pub(crate) enum MetaError {
    ...
    #[error(
        "a halyard::meta tag was hydrated without a MetaContext; call \
...
impl MetaError {
    ...
    pub(crate) fn warn(&self, recovery: &str) {
        halyard::logging::warn!("[halyard] meta: {self}. {recovery}");
    }
}
