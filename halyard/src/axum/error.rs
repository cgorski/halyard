//! What `halyard::axum` does instead of panicking: typed errors, logged, and on the request
//! path an error response.

use crate::axum::route_path::PathError;
use axum::{
    body::Body,
...
/// Logs an error that `halyard::axum` recovered from: with `tracing` when that feature is
/// on, otherwise on standard error, as halyard's other diagnostics are.
pub(crate) fn report(error: &dyn Display, request_id: Option<&str>) {
    ...
    #[cfg(not(feature = "tracing"))]
    {
        ...
        _ = match request_id {
            Some(id) => writeln!(
                std::io::stderr(),
                "[halyard::axum] {error} (request id {id})"
            ),
            None => writeln!(std::io::stderr(), "[halyard::axum] {error}"),
        };
    }
}
