...
use crate::axum::*;
use crate::integration_utils::ExtendResponse;
use crate::router::{
...
/// The options of a test app. Sets the unit tests' shared executor first: they share one
/// global executor, and `halyard_routes`, `generate_route_list` and the static route
/// generator would otherwise set Tokio for the whole binary, under which the resource tests'
/// tasks (spawned outside a Tokio runtime) could not run. Every test here that calls into
/// the integration calls this first.
fn options() -> HalyardOptions {
    crate::server::test_support::init_executor();
...
async fn a_route_list_with_odd_paths_builds_a_router() {
    // first: `generate_route_list` sets the integration's executor if none is set yet
    let options = options();
