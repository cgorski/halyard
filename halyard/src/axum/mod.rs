...
//! Serves a Halyard application with [axum](https://docs.rs/axum): routes for the pages that
//! `halyard::router` lists ([`HalyardRoutes`], [`generate_route_list`]), the handlers that
//! render them in each streaming mode, static routes, and a handler for static files and
//! 404s ([`file_and_error_handler`]). Compiled with the `axum` feature (which implies `ssr`),
//! for a native server running on Tokio.
//!
//! The handlers provide [`ResponseOptions`], the request's [`Parts`](axum::http::request::Parts),
//! a `RequestUrl` and the head's meta context to the rendered application, and set Tokio
//! as the executor when the routes are built.

use crate::integration_utils::{
    BoxedFnOnce, ExtendResponse, PinnedFuture, PinnedStream,
};
use crate::meta::ServerMetaContext;
use crate::router::static_routes::ResolvedStaticPath;
use crate::router::{
...
use halyard_reactive_graph::hydration_context::SsrSharedContext;
...
/// use halyard::prelude::*;
/// use halyard::axum::ResponseOptions;
///
/// #[component]
...
///
/// The route handlers of this module provide it to `halyard::router`, whose `<Redirect/>` calls
...
fn init_executor() {
    let _ = halyard_reactive_graph::executor::Executor::init_tokio();
}
...
/// to route it using [`halyard::router`](crate::router), serving an HTML stream of your application.
///
...
///     let app = Router::new().fallback(halyard::axum::render_app_to_stream(
...
/// Returns an Axum [Handler](axum::handler::Handler) that listens for a `GET` request and tries
/// to route it using [`halyard::router`](crate::router), serving an in-order HTML stream of your application.
...
///         halyard::axum::render_app_to_stream_in_order(|| view! { <MyApp/> }),
///     );
///
///     // run our app with hyper
///     let listener = tokio::net::TcpListener::bind(&addr).await.unwrap();
...
///     req: Request<Body>,
/// ) -> Response {
///     let handler = halyard::axum::render_app_to_stream_with_context(
///         move || {
...
/// Returns an Axum [Handler](axum::handler::Handler) that listens for a `GET` request and tries
/// to route it using [`halyard::router`](crate::router), serving an HTML stream of your application. It allows you
...
///     req: Request<Body>,
/// ) -> Response {
///     let handler = halyard::axum::render_app_to_stream_in_order_with_context(
///         move || {
///             provide_context(id.clone());
///         },
///         || { /* your application here */ },
...
/// Returns an Axum [Handler](axum::handler::Handler) that listens for a `GET` request and tries
/// to route it using [`halyard::router`](crate::router), asynchronously rendering an HTML page after all
...
///     // build our application with a route
///     let app = Router::new()
///         .fallback(halyard::axum::render_app_async(|| view! { <MyApp/> }));
///
///     // run our app with hyper
...
///     req: Request<Body>,
/// ) -> Response {
///     let handler = halyard::axum::render_app_async_with_context(
///         move || {
///             provide_context(id.clone());
///         },
///         || { /* your application here */ },
...
pub struct AxumRouteListing {
    path: String,
    mode: SsrMode,
    methods: Vec<crate::router::Method>,
    #[allow(unused)]
...
impl AxumRouteListing {
    ...
    pub fn new(
        path: String,
        mode: SsrMode,
        methods: impl IntoIterator<Item = crate::router::Method>,
    ...
    pub fn methods(&self) -> impl Iterator<Item = crate::router::Method> + '_ {
...
impl StaticRouteGenerator {
    fn render_route<IV: IntoView + 'static>(
        ...
        let (owner, stream) = crate::integration_utils::build_response(
            app_fn.clone(),
            additional_context,
...
fn static_path(options: &HalyardOptions, path: &str) -> String {
    use crate::integration_utils::static_file_path;
...
where
    ...
    HalyardOptions: FromRef<S>,
{
    /// Adds the routes generated by `halyard::router` to the Axum router.
                    ...
                    SsrMode::Static(_) => {
                        ...
                        add_route(
                            ...
                            |f| {
                                on(
                                    f,
                                    handle_static_route(cx, app_fn, regenerate),
                                )
                            },
                        )
                    }
...
fn http_method(method: crate::router::Method) -> Method {
    match method {
        crate::router::Method::Get => Method::GET,
        crate::router::Method::Post => Method::POST,
        crate::router::Method::Put => Method::PUT,
        crate::router::Method::Delete => Method::DELETE,
        crate::router::Method::Patch => Method::PATCH,
    }
}
