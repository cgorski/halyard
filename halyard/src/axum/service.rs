use crate::axum::{handle_response_inner, PinnedStream};
use axum::{
    body::Body,
    http::{Request, Response, StatusCode},
};
use futures::{stream::once, Future, StreamExt};
use halyard::{config::HalyardOptions, context::provide_context, IntoView};
use std::{
    convert::Infallible,
    pin::Pin,
    task::{Context, Poll},
...
/// Axum's Router set up with a Halyard app, and is provided as a tower [`Service`] to enable composition
...
///
/// [`file_and_error_handler`]: crate::axum::file_and_error_handler
/// [`site_pkg_dir_service`]: crate::axum::site_pkg_dir_service
...
/// # use axum::Router;
/// # use halyard::prelude::*;
/// # use halyard::axum::{HalyardRoutes, generate_route_list};
/// # #[component]
...
/// fn shell(options: HalyardOptions) -> impl IntoView {
///     view! { <App/> }
/// }
///
/// let app = Router::new()
...
///     .fallback_service(
///         halyard::axum::site_pkg_dir_service(&halyard_options).fallback(
///             halyard::axum::ErrorHandler::new(shell, halyard_options),
///         ),
///     );
/// ```
#[derive(Clone, Debug)]
...
impl<SH> ErrorHandler<(), SH> {
    ...
    pub fn new(shell: SH, options: HalyardOptions) -> Self {
        Self {
            additional_context: (),
            shell,
            options,
...
where
    SH: Fn(HalyardOptions) -> IV + 'static + Clone + Send,
    IV: IntoView + 'static,
{
