...
use halyard_dom::helpers::window;
use halyard_server::{ServerAction, ServerMultiAction};
...
/// Automatically turns a server [Action](halyard_server::Action) into an HTML
...
///         <input type="text" name="hefty_arg[first_name]" value="halyard"/>
///         <input
///           type="text"
...
///     assert_eq!(hefty_arg.first_name.as_str(), "halyard");
    ...
    <ServFn as ServerFn>::Client: Client<<ServFn as ServerFn>::Error>,
{
    ...
    _ = halyard_server_fn::redirect::set_redirect_hook(|loc: &str| {
