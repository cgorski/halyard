...
/// Runs each spawned task to completion on a thread of its own, inside a Tokio runtime (the
/// axum integration's tasks use Tokio's file system). Local (`!Send`) tasks are kept but never
/// run (no test here needs one to finish), and never dropped, so that no destructor runs on a
/// thread that is shutting down.
    ...
    fn spawn(&self, fut: PinnedFuture<()>) {
        std::thread::spawn(move || {
            tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
                .expect("a Tokio runtime for a test task")
                .block_on(fut)
        });
    }
...
/// Sets the executor for the test binary (the first test to call this sets it). Every unit
/// test that spawns a task calls this first, so that the executor is the same whichever test
/// runs first (`halyard_routes` sets Tokio when nothing is set yet).
