//! What the server integration shares with any other: building a page's response (its HTML
//! and the scripts that carry its server data) and the static-file paths.

#![allow(clippy::type_complexity)]

use crate::config::HalyardOptions;
use crate::meta::ServerMetaContextOutput;
            ...
            if !prefetches.0.read_value().is_empty() {
                ...
                if let Some(manifest) = use_context::<WasmSplitManifest>() {
                    ...
                    for module in all_prefetches {
                        // to_html() on halyard::meta components registers them with the meta context,
...
fn data_scripts(
    pending_data: Option<
        halyard_reactive_graph::hydration_context::PinnedStream<String>,
    >,
    nonce: String,
) -> PinnedStream<String> {
