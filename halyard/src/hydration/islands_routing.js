	...
	if(NAVIGATION === currentNav) {
		...
		const transition = async () => {
			try {
				...
				for(const island of document.querySelectorAll("halyard-island")) {
					if(!island.$$hydrated) {
...
function diffRange(oldDocument, oldRoot, newDocument, newRoot, oldEnd, newEnd) {
	...
	while (oldDocWalker.nextNode() && newDocWalker.nextNode()) {
		...
		else if (oldNode.nodeType === Node.ELEMENT_NODE && oldNode.tagName === "HALYARD-ISLAND") {
			// TODO: diff the halyard-children 

			// skip over halyard-island otherwise
