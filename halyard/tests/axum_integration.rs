//! The axum integration against a built application (`tests/service_mode`), started as a
//! server: needs the `axum` feature and `cargo halyard` to build the fixture.
#![cfg(feature = "axum")]
...
#[tokio::test]
async fn halyard_options_css_base() -> anyhow::Result<()> {
    let service =
        start_test_service("service_mode", "halyard-options-css-base").await;
    let client = Client::new();
    // should route the css file that was setup using `HalyardOptions`.
    let res = client
...
#[tokio::test]
async fn halyard_options_css_moved() -> anyhow::Result<()> {
    build_services();
    // Note: this test does not test the behavior of `cargo-halyard` but instead will test how the
    ...
    // influence values provided by `HalyardOptions`.
    ...
    let service = start_test_service_with_envs(
        "service_mode",
        "halyard-options-css-base",
        vec![
            (
                "HALYARD_SITE_ROOT",
                site_root.path().to_str().expect("valid utf8"),
            ),
            ("HALYARD_SITE_PKG_DIR", "my/pkg"),
        ],
    )
    .await;
    let client = Client::new();
...
// Killing `cargo halyard watch` may not necessarily kill the underlying server task, so rather
...
fn build_test_service(name: &str) {
    // this assumes the current working dir is at the root of this crate, i.e. `halyard/`.
    ...
    let mut build = cmd
        ...
        .env("HALYARD_OUTPUT_NAME", name)
        .current_dir(&working_dir)
        .spawn()
        .expect("cargo halyard build should start");
    if !build
        .wait()
        .expect("there shouldn't be i/o error")
        .success()
    {
        panic!("failed to run `cargo halyard build`");
    }
}
...
async fn start_test_service_with_envs(
    ...
    let mut child = Command::new(Path::new("target").join("debug").join(name))
        ...
        .kill_on_drop(true)
        .current_dir(&working_dir)
        .env("HALYARD_SITE_ADDR", "127.0.0.1:0")
        // need to manually specify this to avoid mismatch between this value that may be set (e.g.
        // during CI) and the `output-name` defined in Cargo.toml for this relevant project.
        .env("HALYARD_OUTPUT_NAME", name)
        .envs(vars)
