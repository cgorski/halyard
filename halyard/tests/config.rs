use halyard::config::{
    ...
    get_configuration, Env, HalyardOptions,
};
...
async fn get_configuration_from_file_ok() {
    ...
    let config = temp_env::async_with_vars(
        [
            ("HALYARD_OUTPUT_NAME", None::<&str>),
            ("HALYARD_SITE_ROOT", None::<&str>),
            ("HALYARD_SITE_PKG_DIR", None::<&str>),
            ("HALYARD_SITE_ADDR", None::<&str>),
            ("HALYARD_RELOAD_PORT", None::<&str>),
            ("HALYARD_RELOAD_EXTERNAL_PORT", None::<&str>),
        ],
        async { get_configuration(Some(&path_s)).unwrap().halyard_options },
    )
    .await;
...
async fn get_config_from_file_ok() {
    ...
    let config = temp_env::async_with_vars(
        ...
        async { get_config_from_file(&cargo_tmp).unwrap().halyard_options },
    )
    .await;
...
fn get_config_from_str_content() {
    let config = temp_env::with_vars_unset(
        [
            "HALYARD_OUTPUT_NAME",
            "HALYARD_SITE_ROOT",
            "HALYARD_SITE_PKG_DIR",
            "HALYARD_SITE_ADDR",
            "HALYARD_RELOAD_PORT",
            "HALYARD_RELOAD_EXTERNAL_PORT",
        ],
...
async fn get_configuration_from_env() {
    ...
    let config = temp_env::async_with_vars(
        [
            ("HALYARD_OUTPUT_NAME", Some("app-test")),
        ...
        async { get_configuration(None).unwrap().halyard_options },
    )
    .await;
...
#[test]
fn halyard_options_builder_default() {
    let conf = HalyardOptions::builder().output_name("app-test").build();
...
fn environment_variable_override() {
    ...
    let config = temp_env::with_vars(
        [
            ("HALYARD_OUTPUT_NAME", Some("app-test2")),
            ("HALYARD_SITE_ROOT", Some("my_target/site2")),
            ("HALYARD_SITE_PKG_DIR", Some("my_pkg2")),
            ("HALYARD_SITE_ADDR", Some("0.0.0.0:82")),
            ("HALYARD_RELOAD_PORT", Some("8082")),
            ("HALYARD_RELOAD_EXTERNAL_PORT", Some("8082")),
        ],
        || get_config_from_str(CARGO_TOML_CONTENT_OK).unwrap(),
    );
...
env = "{env_value}"
    ...
    let config_from_file =
        get_config_from_file(&path_s).unwrap().halyard_options;

    let config_from_env =
        temp_env::with_vars([("HALYARD_ENV", Some(env_value))], || {
            get_config_from_env().unwrap().halyard_options
        });
