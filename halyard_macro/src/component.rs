...
impl ToTokens for Model {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let props_serializer = if is_island_with_other_props {
            ...
            quote! {
                #[derive(::halyard::serde::Deserialize)]
        ...
        let (
            ...
            #[cfg(feature = "tracing")]
            {
                 ...
                 * ::halyard::tracing), which means that if you're going to use the macro
                 ...
                 *
                 * Removing the feature-check here breaks any project that uses halyard with
                ...
                let instrument = cfg!(feature = "trace-components").then(|| quote! {
                    #[cfg_attr(
                        feature = "tracing",
                        ::halyard::tracing::instrument(level = "info", name = #trace_name, skip_all)
                    )]
                });
                ...
                (
                    ...
                    quote! {
                        let __span = ::halyard::tracing::Span::current();
                    },
                    quote! {
        ...
        let island_serialize_props = if is_island_with_other_props {
            quote! {
                let _halyard_ser_props = ::halyard::serde_json::to_string(&props).expect("couldn't serialize island props");
            }
        } else {
            quote! {}
        };
        ...
        let body_expr = if is_island {
            quote! {
                ::halyard::reactive::owner::Owner::new().with(|| {
                    ::halyard::reactive::owner::Owner::with_hydration(move || {
                        ::halyard::tachys::reactive_graph::OwnedView::new({
        ...
        let component = if *is_transparent {
            ...
            quote! {
                ::halyard::prelude::IntoMaybeErased::into_maybe_erased(
                    ::halyard::reactive::graph::untrack_with_diagnostics(
                        move || {
                            #tracing_guard_expr
                            #tracing_props_expr
                            #body_expr
        ...
        let component = if is_island {
            ...
            quote! {
                ::halyard::tachys::html::islands::Island::new(
        ...
        let destructure_props = if no_props {
            ...
            let wrapped_children = if is_island_with_children {
                quote! {
                    use halyard::tachys::view::any_view::IntoAny;
                    let children = Box::new(|| {
                        let sc = ::halyard::reactive::owner::Owner::current_shared_context().unwrap();
                        let prev = sc.get_is_hydrating();
                        let owner = ::halyard::reactive::owner::Owner::new();
                        let value = owner.clone().with(|| {
                            ::halyard::reactive::owner::Owner::with_no_hydration(move || {
                                ::halyard::tachys::reactive_graph::OwnedView::new({
                                    ::halyard::tachys::html::islands::IslandChildren::new_with_on_hydrate(
                                        children(),
                                        {
        ...
        let binding = if is_island {
                ...
                || is_island_with_other_props
            {
                ...
                let children = if is_island_with_children {
                    quote! {
                        .children({
                            let owner = halyard::reactive::owner::Owner::current();
                            Box::new(move || {
                            use halyard::tachys::view::any_view::IntoAny;
                            ::halyard::tachys::html::islands::IslandChildren::new_with_on_hydrate(
                                (),
            ...
            let deserialize_island_props = if is_island_with_other_props {
                quote! {
                    let props = el.dataset().get(::halyard::wasm_bindgen::intern("props"))
                        .and_then(|data| ::halyard::serde_json::from_str::<#props_serialized_name>(&data).ok())
            ...
            let hydrate_fn_inner = quote! {
                ::halyard::tachys::hydration::take_hydration_failure();
                let state = {
                    #deserialize_island_props
                    let island = #name(#island_props);
                    island.hydrate_from_position::<true>(&el, ::halyard::tachys::view::Position::Current)
                };
                if ::halyard::tachys::hydration::take_hydration_failure() {
                    // the server-rendered DOM of this island did not match: discard the
                    // half-hydrated state and render the island on the client instead
                    drop(state);
                    #deserialize_island_props
                    ::halyard::mount::__island_client_render_fallback(&el, #name(#island_props));
                } else {
                    // TODO better cleanup
            ...
            if *is_lazy {
                ...
                quote! {
                    #[::halyard::prelude::lazy]
                    #[allow(non_snake_case)]
                    fn #outer_name (el: ::halyard::web_sys::HtmlElement) {
                        #hydrate_fn_inner
                    }

                    #[::halyard::wasm_bindgen::prelude::wasm_bindgen(
                        wasm_bindgen = ::halyard::wasm_bindgen,
                        wasm_bindgen_futures = ::halyard::__reexports::wasm_bindgen_futures
                    )]
                    #[allow(non_snake_case)]
                    pub async fn #hydrate_fn_name(el: ::halyard::web_sys::HtmlElement) {
                        #outer_name(el).await
                    }
                }
            } else {
                quote! {
                    #[::halyard::wasm_bindgen::prelude::wasm_bindgen(wasm_bindgen = ::halyard::wasm_bindgen)]
                    #[allow(non_snake_case)]
                    pub fn #hydrate_fn_name(el: ::halyard::web_sys::HtmlElement) {
                        #hydrate_fn_inner
                    }
                }
            }
        ...
        let output = quote! {
            ...
            #[doc = ""]
            #docs_and_prop_docs
            #[derive(::halyard::typed_builder_macro::TypedBuilder #props_derive_serialize)]
            //#[builder(doc)]
            #[builder(crate_module_path=::halyard::typed_builder)]
            #[allow(non_snake_case)]
            ...
            impl #impl_generics ::halyard::component::Props for #props_name #generics #where_clause {
            ...
            /*impl #impl_generics ::halyard::DynAttrs for #props_name #generics #where_clause {
                fn dyn_attrs(mut self, v: Vec<(&'static str, ::halyard::Attribute)>) -> Self {
                    #dyn_attrs_props
                    self
                }
            } */
...
impl Docs {
    pub fn new(attrs: &[Attribute]) -> Self {
        ...
        // todo fix docs stuff
        const RSX_START: &str = "# ::halyard::view! {";
        const RSX_END: &str = "# };";
    ...
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let into = if self.into {
            if !self.strip_option {
                let ty = &self.ty;
                quote! {
                    fn transform<__IntoReactiveValueMarker>(value: impl ::halyard::prelude::IntoReactiveValue<#ty, __IntoReactiveValueMarker>) -> #ty {
                ...
                quote! {
                    fn transform<__IntoReactiveValueMarker>(value: impl ::halyard::prelude::IntoReactiveValue<#ty, __IntoReactiveValueMarker>) -> Option<#ty> {
