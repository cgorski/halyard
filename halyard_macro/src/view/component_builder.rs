...
pub(crate) fn component_to_tokens(
    ...
    for (_, attr) in attrs.iter_mut().enumerate().filter(|(idx, attr)| {
        ...
        if optional {
            optional_props.push(quote! {
                props.#name = { #value }.map(::halyard::prelude::IntoReactiveValue::into_reactive_value);
            })
        } else {
    ...
    let spreads = (!(spreads.is_empty())).then(|| {
        if cfg!(feature = "__internal_erase_components") {
            quote! {
                .add_any_attr({
                    vec![#(::halyard::attr::any_attribute::IntoAnyAttribute::into_any_attr(#spreads),)*]
                })
            }
        } else {
            quote! {
    ...
    let mut component = quote! {
        {
            ...
            ::halyard::component::component_view(
                #[allow(clippy::needless_borrows_for_generic_args)]
                &#name,
                {
                    let mut props = ::halyard::component::component_props_builder(&#name #generics)
                        #(#required_props)*
                        #(#slots)*
                        #children
                        .build();
...
/// This is to work with all the variants of the halyard::children::ToChildren::to_children trait.
...
pub fn maybe_optimised_component_children(
    ...
    Some(quote_spanned! {children.span()=>
        .children({
            ...
            ::halyard::children::ToChildren::to_children(::halyard::children::ChildrenOptContainer(#children))
        })
    })
}
