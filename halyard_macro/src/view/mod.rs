...
fn inert_element_to_tokens(
    ...
    quote! {
        ::halyard::tachys::html::InertElement::new(#html)
    }
}
...
fn inert_svg_element_to_tokens(
    ...
    quote! {
        ::halyard::tachys::svg::InertElement::new(#html)
    }
}
...
fn element_children_to_tokens(
    ...
    if children.is_empty() {
        ...
        Some(quote! {
            .child(
                ::halyard::tachys::view::iterators::StaticVec::from(vec![#(
                    ::halyard::prelude::IntoMaybeErased::into_maybe_erased(#children)
                ),*])
            )
        })
...
fn node_to_tokens(
    ...
    match node {
        ...
        Node::Doctype(node) => {
            ...
            Some(quote! { ::halyard::tachys::html::doctype(#value) })
        }
        ...
        Node::Block(block) => {
            ...
            // halyard/Cargo.toml), wrap reactive child closures in the
            ...
            {
                Some(quote! {
                    ::halyard::prelude::IntoRender::into_render(
                        ::halyard::__as_shared_reactive_fn(#block)
                    )
                })
            } else {
                Some(
                    quote! { ::halyard::prelude::IntoRender::into_render(#block) },
                )
            }
        }
...
pub(crate) fn element_to_tokens(
    ...
    if is_component_node(node) {
        ...
        let name = if is_custom {
            ...
            quote_spanned! { node.name().span() => ::halyard::tachys::html::element::#custom(#name) }
            ...
            quote_spanned! { node.name().span() => ::halyard::tachys::svg::#name() }
        } else if is_math_ml_element(&tag) {
            parent_type = TagType::Math;
            quote_spanned! { node.name().span() => ::halyard::tachys::mathml::#name() }
            ...
            match parent_type {
                TagType::Unknown => {
                    ...
                    quote_spanned! { node.name().span() =>
                        ::halyard::tachys::html::element::#name()
                    }
                }
                TagType::Html => {
                    quote_spanned! { node.name().span() => ::halyard::tachys::html::element::#name() }
                }
                TagType::Svg => {
                    quote_spanned! { node.name().span() => ::halyard::tachys::svg::#name() }
                }
                TagType::Math => {
                    quote_spanned! { node.name().span() => ::halyard::tachys::math::#name() }
                }
            }
        } else {
            parent_type = TagType::Html;
            quote_spanned! { name.span() => ::halyard::tachys::html::element::#name() }
        };
        ...
        NodeAttribute::Attribute(node) => {
                ...
                || (tag_type == TagType::Svg && name != "inner_html")
            {
                    ...
                    && node.value().and_then(value_to_string).is_none()
                {
                    ...
                    emit_error!(span, "Combining a global class (view! { class = ... }) \
    ...
    match &node.key {
                    ...
                    NodeNameFragment::Ident(id) => {
                        ...
                        if id == "let" || id == "clone" {
                            ...
                            if key_name == "class" || key_name == "style" {
                                Some(
                                    quote! { ::halyard::tachys::html::#key::#key(#value) },
                                )
                                ...
                                Some(
                                    quote! { ::halyard::tachys::html::attribute::#key(#value) },
                                )
                            } else if multipart {
                                ...
                                Some(
                                    quote! { ::halyard::tachys::html::attribute::custom::custom_attribute(#key_name, #value) },
                                )
                            } else {
                                Some(
                                    quote! { ::halyard::tachys::html::attribute::#key(#value) },
                            ...
                            Some(
                                quote! {
                                    ::halyard::tachys::html::directive::directive(
                                        #key,
                            ...
                            let key = key
                                ...
                                .replacen("class:", "", 1);
                            Some(
                                quote! { ::halyard::tachys::html::#id::#id((#key, #value)) },
                            )
                        } else if id == "prop" {
                            let value = attribute_value(node, false);
                            let key = &node.key.to_string();
                            ...
                            Some(
                                quote! { ::halyard::tachys::html::property::#id(#key, #value) },
                            )
                        } else if id == "on" {
                            let key = &node.key.to_string();
                            ...
                            Some(
                                quote! { ::halyard::tachys::html::event::#on(#ty, #handler) },
                            )
                        } else {
                            abort!(
                                id.span(),
        ...
        _ => after_spread.then(|| {
            ...
            if name == "class" || name == "style" {
                quote! {
                    ::halyard::tachys::html::#key::#key(#value)
                }
            }
            else if name.contains('-') && !name.starts_with("aria-") {
                quote! {
                    ::halyard::tachys::html::attribute::custom::custom_attribute(#name, #value)
                }
            }
            else if name == "node_ref" {
                quote! {
                    ::halyard::tachys::html::node_ref::#key(#value)
                }
            }
            else {
                quote! {
                    ::halyard::tachys::html::attribute::#key(#value)
                }
            }
        }),
    }
...
pub(crate) fn two_way_binding_to_tokens(
    ...
    if name == "group" {
        quote! {
            .bind(halyard::tachys::reactive_graph::bind::#ident, #value)
        }
    } else {
        quote! {
            .bind(::halyard::attr::#ident, #value)
        }
    }
}
...
pub(crate) fn event_type_and_handler(
    ...
    let event_type = quote! {
        ::halyard::tachys::html::event::#event_type
    };
    let event_type = if options.captured {
        ...
        quote! { ::halyard::tachys::html::event::#capture(#event_type) }
    } else {
        event_type
    };
    ...
    let event_type = if options.undelegated {
        ...
        quote! { ::halyard::tachys::html::event::#undelegated(#event_type) }
    } else {
        event_type
    };
            ...
            if let Expr::Array(ExprArray { elems, .. }) = name {
                return elems
                    ...
                    .map(|elem| match elem {
                        ...
                        _ => abort!(elem.span(), "invalid name"),
                    })
                    .collect();
            }
        }
        ...
        Some(value) => match &value.value {
            KVAttributeValue::Expr(expr) => {
                ...
                if matches!(expr, Expr::Lit(_)) || !is_attribute_proper {
                    if wrap_closure {
                        quote! {
                            ::halyard::__as_shared_reactive_fn(#expr)
                        }
                    } else {
                        quote! {
                            #expr
                    ...
                    quote! {
                        ::halyard::prelude::IntoAttributeValue::into_attribute_value(
                            ::halyard::__as_shared_reactive_fn(#expr)
                        )
                    }
                } else {
                    quote! {
                        ::halyard::prelude::IntoAttributeValue::into_attribute_value(#expr)
                    }
                }
            }
            ...
            KVAttributeValue::InvalidBraced(block) => {
                if is_attribute_proper {
                    quote! {
                        ::halyard::prelude::IntoAttributeValue::into_attribute_value(#block)
                    }
                } else {
                    quote! {
                        #block
...
pub(crate) fn full_path_from_tag_name(tag_name: &NodeName) -> Option<ExprPath> {
    match tag_name {
        ...
        _ => {
            let span = tag_name.span();
            emit_error!(span, "punctuated names not allowed in slots");
            None
        }
    }
}
