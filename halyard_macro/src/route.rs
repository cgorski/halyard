//! `path!` and `#[lazy_route]`: the router's macros (see their entry points in `lib.rs`).
...
impl ToTokens for Segment {
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
        match self {
            Segment::Wildcard(s) => {
                tokens
                    .extend(quote! { ::halyard::router::WildcardSegment(#s) });
            }
            Segment::Static(s) => {
                tokens.extend(quote! { ::halyard::router::StaticSegment(#s) });
            }
            Segment::Param(p) => {
                tokens.extend(quote! { ::halyard::router::ParamSegment(#p) });
            }
            Segment::OptionalParam(p) => {
                tokens.extend(
                    quote! { ::halyard::router::OptionalParamSegment(#p) },
                );
            }
        }
    }
        ...
        syn::parse::<ImplItem>(
            quote! {
                async fn preload() {
                    ...
                    // but we don't require a `hydrate` feature on halyard
