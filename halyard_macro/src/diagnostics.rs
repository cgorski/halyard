//! Minimal diagnostics for halyard's proc macros.
//!
...
//!   recorded error with [`syn::Error::to_compile_error`].
//!
    ...
    F: FnOnce() -> proc_macro::TokenStream,
{
    ...
    match result {
        ...
        Ok(_) => render(errors).into(),
        Err(payload) if payload.is::<AbortNow>() => render(errors).into(),
        Err(payload) => resume_unwind(payload),
    }
}

fn render(errors: Vec<syn::Error>) -> TokenStream {
...
fn check_in_entry_point() {
    if DEPTH.with(Cell::get) == 0 {
        panic!(
            "halyard_macro: emit_error!/abort! used \
...
macro_rules! __diag_error {
    ($span:expr, $($rest:tt)*) => {{
        ...
        use $crate::diagnostics::span::{SpanDiagnostic as _, TokensDiagnostic as _};
        let (msg, suggestions) = __diag_message!($($rest)*);
        let msg = $crate::diagnostics::format_message(msg, &suggestions);
...
macro_rules! abort {
    ($span:expr, $($rest:tt)*) => {
        $crate::diagnostics::abort(__diag_error!($span, $($rest)*))
    };
}
...
macro_rules! abort_call_site {
    ($($rest:tt)*) => {
        abort!($crate::diagnostics::proc_macro2::Span::call_site(), $($rest)*)
    };
}
...
macro_rules! emit_error {
    ($span:expr, $($rest:tt)*) => {
        $crate::diagnostics::emit_error(__diag_error!($span, $($rest)*))
    };
}
...
mod tests {
    ...
    fn run(f: impl FnOnce() -> TokenStream) -> String {
        ...
        match result {
            ...
            Ok(_) => render(errors).to_string(),
            Err(payload) if payload.is::<AbortNow>() => {
                render(errors).to_string()
            }
            Err(payload) => resume_unwind(payload),
        }
    }
    ...
    #[test]
    fn abort_renders_compile_error() {
        let out = run(|| {
