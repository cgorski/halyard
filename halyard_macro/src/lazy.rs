...
pub fn lazy_impl(args: proc_macro::TokenStream, s: TokenStream) -> TokenStream {
    ...
    let is_wasm = cfg!(feature = "hydrate");
    if is_wasm {
        let mut fun = fun;
        ...
        if was_async {
            ...
            fun.block.stmts.push(Stmt::Expr(parse_quote! {
                ::std::boxed::Box::pin(::halyard::__reexports::send_wrapper::SendWrapper::new(async move {
                    #( #stmts )*
                }))
            }, None));
        ...
        quote! {
            #[::halyard::wasm_split::wasm_split(
                #unique_name,
                wasm_split_path = ::halyard::wasm_split,
        ...
        statements.push(parse_quote! {
            ::halyard::prefetch_lazy_fn_on_server(#unique_name_str);
        });
...
pub fn lazy_preload_impl(s: proc_macro::TokenStream) -> TokenStream {
    ...
    let preload_call = if cfg!(feature = "hydrate") {
        quote! {
            ::halyard::task::spawn_local(async move {
    ...
    quote! {{
        use ::halyard::prelude::Set;

        let (loaded, set_loaded) = ::halyard::prelude::signal(false);
        #preload_call
        loaded
    }}
    .into()
