...
///    If you insert a signal here, Halyard will create an effect to update the DOM whenever the value changes.
...
fn view_macro_impl(tokens: TokenStream, template: bool) -> TokenStream {
    ...
    if template {
        quote! {
            ::halyard::prelude::ViewTemplate::new(#output)
        }
    } else {
        output
    }
...
/// Annotates a function so that it can be used with your template as a Halyard `<Component/>`.
///
/// The `#[component]` macro allows you to annotate plain Rust functions as components
/// and use them within your Halyard [view](crate::view!) as if they were custom HTML elements. The
...
///
/// Here’s how you would define and use a simple Halyard component which can accept custom properties for a name and age:
///
/// ```rust
/// # use halyard::prelude::*;
...
/// ```
///
/// Here are some important details about how Halyard components work within the framework:
///
...
/// `islands` feature of Halyard. Apart from the macro name,
...
///    in fact they're all bundled into a single `<halyard-children>` HTML element.
...
/// within your Halyard [`component`](macro@crate::component) properties. The struct can contain any number
...
///
/// Here’s how you would define and use a simple Halyard component which can accept a custom slot:
/// ```rust
/// # use halyard::prelude::*;
/// use std::time::Duration;
///
...
/// of which can be found in the [`halyard_server_fn::codec`](../halyard_server_fn/codec/index.html) module.
///
...
/// 1. [`IntoReq`](../halyard_server_fn/codec/trait.IntoReq.html): The client serializes the [`ServerFn`](../halyard_server_fn/trait.ServerFn.html) argument type into an HTTP request.
/// 2. The [`Client`](../halyard_server_fn/client/trait.Client.html) sends the request to the server.
/// 3. [`FromReq`](../halyard_server_fn/codec/trait.FromReq.html): The server deserializes the HTTP request back into the [`ServerFn`](../halyard_server_fn/client/trait.Client.html) type.
/// 4. The server calls calls [`ServerFn::run_body`](../halyard_server_fn/trait.ServerFn.html#tymethod.run_body) on the data.
/// 5. [`IntoRes`](../halyard_server_fn/codec/trait.IntoRes.html): The server serializes the [`ServerFn::Output`](../halyard_server_fn/trait.ServerFn.html#associatedtype.Output) type into an HTTP response.
/// 6. The server integration applies any middleware from [`ServerFn::middleware`](../halyard_server_fn/middleware/index.html) and responds to the request.
/// 7. [`FromRes`](../halyard_server_fn/codec/trait.FromRes.html): The client deserializes the response back into the [`ServerFn::Output`](../halyard_server_fn/trait.ServerFn.html#associatedtype.Output) type.
///
...
///     - [`ServerFnError`](../halyard_server_fn/error/enum.ServerFnError.html) can be generic over some custom error type. If so, that type should implement
...
pub fn server(args: proc_macro::TokenStream, s: TokenStream) -> TokenStream {
    // `server_macro_impl` is already `Result`-based; no `entry_point` needed
    match halyard_server_fn_macro::server_macro_impl(
        args.into(),
        s.into(),
        Some(syn::parse_quote!(::halyard::server_fn)),
...
///
/// This should be used in tandem with a suitable build process, such as `cargo halyard --split`.
///
/// ```rust
/// # use halyard_macro::lazy;
///
/// #[lazy]
