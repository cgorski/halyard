//! A macro to make path definitions easier with [`halyard_router`].
//!
//! [`halyard_router`]: https://docs.rs/leptos_router/latest/leptos_router/components/fn.Route.html
        ...
        for input in self.input.by_ref() {
            match input {
                ...
                other => {
                    return Err(syn::Error::new(
                        other.span().into(),
                        "expected a string literal path like `\"/foo/:bar\"`",
                    ));
                }
            }
        }
...
impl Segment {
    ...
    fn ensure_valid(&self) -> syn::Result<()> {
        let err = |msg: String| Err(syn::Error::new(Span::call_site(), msg));
        match self {
            Self::Wildcard(s) if !Self::is_valid(s) => {
                err(format!("Invalid wildcard segment: {s}"))
            }
            Self::Static(s) if !Self::is_valid(s) => {
                err(format!("Invalid static segment: {s}"))
            }
            Self::Param(s) if !Self::is_valid(s) => {
                err(format!("Invalid param segment: {s}"))
            }
            _ => Ok(()),
        }
    }
...
impl ToTokens for Segment {
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
        match self {
            Segment::Wildcard(s) => {
                tokens.extend(quote! { halyard_router::WildcardSegment(#s) });
            }
            Segment::Static(s) => {
                tokens.extend(quote! { halyard_router::StaticSegment(#s) });
            }
            Segment::Param(p) => {
                tokens.extend(quote! { halyard_router::ParamSegment(#p) });
            }
            Segment::OptionalParam(p) => {
                tokens.extend(
                    quote! { halyard_router::OptionalParamSegment(#p) },
                );
            }
        }
    }
...
/// ```rust
/// use halyard::prelude::*;
/// use halyard_router::{lazy_route, LazyRoute};
///
    ...
    match item {
        ...
        Some(fun) => {
            ...
            Ok(quote! {
                ...
                #[::halyard::lazy]
                fn #lazy_view_ident(#first_arg_pat: #self_ty) -> ::halyard::prelude::AnyView {
                    #body
                }
